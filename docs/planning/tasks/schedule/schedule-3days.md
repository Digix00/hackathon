...
| `feat/prototype` | ✅ 完了 | iOSプロトタイプ（UIテーマ、画面コンポーネント） |
...
[iOS初期化] ──────────▶ [iOS BLE調査・設計] ──────▶ [iOS BLE実装] ─────────────▶ [BLE すれ違い検知]
                                                   │                                  │
                                                   └──────────────▶ [バックグラウンド検証] │

[Backend初期化] ─────▶ [Backend API設計] ───────▶ [BLE トークンAPI] ─────────▶ [Encounter API]
      │                                              │                                  │
      └──────────────▶ [DBスキーマ・ログ基盤] ───────┴──────────────────────────▶ [DB保存・統合テスト]

[Android初期化] ─────▶ [Android BLE基盤] ─────────▶ [Room/Retrofit設定] ───────▶ [レビュー・補完]
```
...
- BLE すれ違い検知（iOS）
- 歌詞投稿（iOS + Backend）
- Backend BLE トークン API
- Backend Encounter API
