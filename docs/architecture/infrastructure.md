...
| Cloud Run Service（worker） | 通知バッチ処理。Cloud Scheduler から HTTP POST でトリガー、スケールゼロ対応 | ✅ |
| Cloud Run Jobs | DB マイグレーション（migrate）、Lyria 生成ジョブ | ✅ |
...
└── main/                     # 全リソース管理
    ...
    ├── cloudrun.tf            # Cloud Run Services（API, worker）+ Cloud Run Jobs（migrate）
