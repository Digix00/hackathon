...
- トラック系レスポンスの `preview_url` は外部音楽サービス由来の補助情報であり、取得時点で利用不能または `null` の場合がある。MVP では永続キャッシュ対象とは限らず、都度取得または未設定を許容する。
...
ユーザー初回登録。
`avatar_url` は MVP フェーズでは外部 URL をそのまま保存する（DB の `avatar_file_id` ではなく URL 文字列として直接扱う。ファイルアップロード用エンドポイントは MVP スコープ外）。
