...
4. **バックエンド変更が含まれる場合は、必要に応じて `make fmt` / `make generate-code` を実行する（詳細は後述）。**
...
## バックエンド変更時の `make fmt` / `make generate-code`

バックエンドに変更がある場合、コミット前に以下を検討・実行する。

- **`make fmt`**: `backend/` 配下の Go ファイルに変更がある場合は原則実行する。
- **`make generate-code`**: Swagger 注釈や API 定義に影響しそうな変更（例: `backend/cmd/server`、`backend/internal/handler`、`backend/internal/usecase`、`backend/internal/domain`、`backend/docs` など）が含まれる場合は実行する。  
